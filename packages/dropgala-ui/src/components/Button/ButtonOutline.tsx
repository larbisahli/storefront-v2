...
const ButtonOutline: React.FC<React.PropsWithChildren<ButtonProps>> = ({
  className,
  label,
  loading = false,
  size = 'normal',
  ...
  const renderButton = () => (
    <button
      ...
      className={cn(
        'scroll-mt-320px flex items-center justify-center flex-shrink-0 font-normal w-auto uppercase',
        ...
        className,
        {
          'text-gray-500 bg-gray-300 cursor-not-allowed hover:bg-gray-300':
            disabled,
          'border border-solid border-gray-500': !disabled
        }
      )}
      disabled={disabled}
      type={type}
